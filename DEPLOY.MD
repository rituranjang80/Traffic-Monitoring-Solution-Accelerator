...
## Step 6A. Set up  configuration appsetting.json in web API
        ...
        * Enter the SQL sever connection strings details       
			...
			To fetch SQL sever Server details 
			  (a). Go to SQL databases in azure portal
			  (b). click on your databaename in my case MicrosoftTrafficMgmt
			  (c). click on overviwe than click on connection strings 
			  (d). copy ther the server name,catalog name user 
    ...
    "CosmosDb": {
		...
		To fetch web CosmosDb details
			...
			(b). Copy the database  under cntainers section
			(c). Select "Key" option on left
			(d). Copy URI,PRIMARY KEY, and PRIMARY CONNECTION STRING 
