...
## Step  4 Setup configuration for Ml models
		...
		##	Edit config file for HighwayMonitoring_ML/TrafficMonitoring_Stream/app model
		...
		*      Cosmos db connetion
		...
		##      sql Connection string
				...
				To fetch SQL sever Server details 
				  ...
				  (d). Copy the server name,DATABASE/Catalog name, user and fill into above SQL Connection string
				  (e). Enter the password of sql server


		
		##	Edit Config file for  HighwayMonitoring_ML/TrafficMonitoring_Video/app model
		...
		*       Blob Storage Connection
		...
		##	Edit Config file for  HighwayMonitoring_ML/AccidentDetection_Live/app Model
		...
		*       Cosmos db Connection
		...
		##	Edit Config file for  HighwayMonitoring_ML/AccidentDetection_Video/app Model
		*       Front end api url 
...
## Step 6A. Set up  configuration appsetting.json in web API
        ...
        * Enter the SQL sever connection strings details       
			...
			To fetch SQL sever Server details 
			  ...
			  (b). Click on your databasename in my case MicrosoftTrafficMgmt
			  (c). Click on overview then click on connection strings 
			  (d). Copy the server name,catalog name, user and fill into above SQL Connection string 
			  (e). Enter the password of sql server
...
## Appendix - Use of media service for Live stream .
			 ...
			 please refer this video https://www.youtube.com/watch?v=o2uiYLT6KU8 
			1) Create a media service in azure portal
			2) Open the Media Service->Overview->Encoding->uploading a video
			3) After video upload select the file than click on job 
				a) Job page open enter the details
				a) check the option encoding
				b) Built in present name (select the option-> H254MultipleBitrate1080P)
			4) Open the new created encoding video file 
			5) Click on new streaming locator
			6) A new streaming locator window open enter the expiry date (current date+2 years or which you want)
			8) Now encoding video show on portal
			9) click on streaming URL under section Streaming locators (1)
			10) A new popup Streaming URLs window open copy the HLS URL in show advance section 
			11)Open the HighwayMonitoring website and camera configuration page add paste HLS URL in endpoint
